...
this iterative process can surface effective variants that exploit agentic and
multi-agent workflows under a small search budget, and (ii) whether
final-output-only evaluation misses leakage and degradation revealed by the
same process in internal channels. We adopt an AutoResearch-style loop,
...
goals across four systems: AgentDojo, AutoGen, CrewAI, and LangGraph, configured with a shared abstract pipeline so that results are comparable across frameworks. Throughout, the attacker only injects content into untrusted
